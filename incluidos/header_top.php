<link rel="shortcut icon" href="images/icono-hospital.png">
<div class="header clearfix row">
  <div class="eight columns header_left">
    <a href="index.php"><img src="images/logo1.jpg"/></a>
  </div>
  <div class="eight columns">
    <div class="header_right">
      <div class="header_contacts clearfix">
        <div class="header_phone">+54 0294 442-6117</div>
        <div class="header_mail">secretaria@hospitalbariloche.com.ar</div>
      </div>

        <p style="font-size: 30px; ">Pagina en construccion!</p>

        <a href="https://www.facebook.com/?stype=lo&jlou=Afc2zVBPHTaksx-hAPg4FBsoEKr3wpuUM31A52HlKgX1bX1Worpx7rdK4D3vJyU2I8tn9NWDUTymATkf3mgrxazoth7bSl_c6FOCeHAWObaK_A&smuh=27173&lh=Ac-m0clfUZ7LquC0" class="header_soc_fb" id="soc_fb">Facebook</a>



      </div>
    </div>
  </div>
</div>
